...
namespace Wuyi.Common
{
    ...
    public class HttpServer
    {
       private TcpListener _myListener;
        ...
        private Thread _th; //监听进程

        private int _maxConnect=10;
        ...
        public DelegateException OnException;
        ...
        public void Start()
        {
            try
            {
                _myListener = new TcpListener(IPAddress.Parse("0.0.0.0"), this._port); //指定端口及IP

                this._myListener.Start(MaxConnect); //打开监听端口

                IsRuning = true;

                this._th = new Thread(new ThreadStart(StartListen));

                this._th.IsBackground = true;

                this._th.Start(); //启动监听线程
        ...
        private void StartListen()
        {
            while (IsRuning)
            {
                try
                {
                    TcpClient client = _myListener.AcceptTcpClient(); //阻塞当前线程直到接收到连接请求
