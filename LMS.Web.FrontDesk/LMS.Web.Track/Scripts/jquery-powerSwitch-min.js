(function(b, c, d) {
	...
	c.fn.powerSwitch = function(k) {
		...
		c.each(["disabled", "prev", "play", "pause", "next"], function(C, F) {
			F = c.trim(F);
			var G = F.slice(0, 1).toUpperCase() + F.slice(1),
				E = "class" + G,
				D = z.classPrefix.slice(-1);
			if (z[E] === d) {
				if (z.classPrefix) {
		...
		var B = z.indexSelected || -1,
			h = parseInt(z.number) || 1,
			f = null,
			w = null,
			v = c(),
			l = 0;
						...
						if (z.container && l > h) {
							if (C == 0) {
								...
								if (F < 0) {
									...
									c.powerSwitch.animation(null, null, c.extend({}, z, {
										animation: "none"
									}));
									F = B + l - h
								}
		...
		if ((x == false && z.toggle == false && t == false) || (t == true && !z.classDisabled)) {
			if (z.container && t == false) {
				...
				z.container.append(i).delegate("a", "click", function() {
					var C = c(this).attr("data-type"),
						E = z["class" + C.slice(0, 1).toUpperCase() + C.slice(1)],
						D = B;
					switch (C) {
					case "prev":
						z.prevOrNext = c(this);
